...
if (Device.isIOS()) {
	println("Re-installing and launching app for the first time in an iOS device after uninstalling it")
	Mobile.startApplication(GlobalVariable.G_iOS_App, false)
}
if (Device.isAndroid()) {
	println("Re-installing and launching app for the first time in an Android device after uninstalling it")
	Mobile.startApplication(GlobalVariable.G_Android_App, false)
	Mobile.verifyElementExist(findTestObject('Object Repository/Welcome_screen/Android-Welcome_appTitle'), GlobalVariable.G_Timeout_long) 
}
